...
\end{itemize}
TurboGrid reports these metrics directly for every generated mesh. Figure~\ref{fig:turbogrid_mesh} shows representative views of the GV TurboGrid mesh for one configuration in the screening matrix. The full vane row shows the structured O-grid wrapping each blade in the cylindrical reference frame. The single-blade close-up shows the boundary-layer cells on the suction surface with the targeted $y^+ \approx 1$ resolution.

\begin{figure}[h!]
  \centering
  \begin{subfigure}[t]{0.55\textwidth}
    \centering
    \includegraphics[width=\textwidth]{Figures/Mesh/TurboGrid_GV_row.png}
    \caption{Full GV row.}
    \label{fig:turbogrid_gv_row}
  \end{subfigure}
  \hfill
  \begin{subfigure}[t]{0.40\textwidth}
    \centering
    \includegraphics[width=\textwidth]{Figures/Mesh/TurboGrid_GV_blade.png}
    \caption{Single-blade close-up.}
    \label{fig:turbogrid_gv_blade}
  \end{subfigure}
  \caption[GV TurboGrid mesh]{Representative TurboGrid mesh of a guide-vane configuration. The structured O-grid topology is preserved across all 18 GV configurations and the RDT rotor mesh.}
  \label{fig:turbogrid_mesh}
\end{figure}

\paragraph{Local orthogonality excursions in CS configurations.}
Some CS configurations produced local cells with orthogonality angles below the TurboGrid Min Limit of $20^\circ$, with the smallest values around $17^\circ$. The affected cells are localised in regions where the inlet-to-leading-edge distance varies along the span. This is intrinsic to the constant-solidity construction: the chord scales linearly with $r$, so the LE moves axially across the span and the upstream block becomes mildly skew. The fraction of affected cells is approximately $0.004\,\%$ of the global cell count, and the corresponding impact on the integral output quantities is negligible. The affected meshes are retained for the screening campaign on this basis. The remaining mesh-quality metrics (face angles, expansion ratio, determinant, skewness) satisfy the TurboGrid limits everywhere in the GV and RDT meshes.
...
\begin{figure}[h!]
  ...
  \caption[Post-processing planes used in the GCI study]{Post-processing planes for the GCI study on the swirling-flow simulation. The planes are GVStart, the GVEnd-series along the conical contraction, BendStart, BendEnd and ConeEnd, with normals aligned with the local duct centreline. Plane labels follow Table~\ref{tab:planes}.}
