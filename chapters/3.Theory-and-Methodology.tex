...
\end{center}
%RETT: Ikke skrive noe slik direkte kobling mot prosjektarbeidet, fordi prosjektarbeidet ikke er publisert. skriv heller in the previous project work eller noe lignende som bygger på forrige semester
...
\end{equation}
Throughout this thesis the symbols $C_m$ and $C_u$ are used as the preferred notation for the mean axial and tangential velocity components. Labels such as \texttt{Velocity.Axial}, \texttt{Velocity.Circumferential}, $U_x$ and $U_t$ are only used when referring explicitly to CFD monitor expressions or post-processing variables.
...
\begin{figure}[h!]
    \centering
    \fbox{\includegraphics[width=0.8\linewidth]{Figures/Foil2d/Airfoil_basics.png}}
...
\begin{figure}[h!]
  \centering
  \fbox{\includegraphics[width=0.40\textwidth]{Figures/Theory/Tip_vortex}}
...
%ha med det bildet som jeg hadde i prosjektoppgaven med 2d foil og ulike vinkler
 
% ---------------------------------------------------------------------
\section{Geometric construction of the guide vanes}
\label{sec:theory_geometric_construction}
%Ikke skrive det som om at det er laget i project work, siden projct work ikke er publisert. Jeg skal legge ved den matlab koden den henviser til, og derfor vil den ligge i appendix. så ikke noe slik sitering..
...
\label{eq:dean}
\end{equation}
where $D_h$ is the hydraulic diameter of the duct and $R_c$ is the radius of curvature of the bend centreline. For the present geometry the duct diameter at the bend is $D = 0.4$~m and the bend centreline radius is $R_c \approx 0.50$~m, giving a curvature ratio $R_c/D = 1.24$. With Reynolds numbers around $\mathrm{Re} \sim 10^6$ in the duct, the Dean number is well into the turbulent regime where the Dean cells are fully developed and may further interact with the upstream-imposed swirl. Three regimes are commonly distinguished. For $\mathrm{De} \lesssim 100$, the bend secondary flow is laminar and weak. For intermediate Dean numbers, instabilities and additional secondary cells emerge. For high Reynolds numbers, the secondary motion is fully turbulent and is dominated by two large counter-rotating Dean cells, with additional small-scale features near the inner radius where separation is most likely.
...
%KOMMENTERE NOE PÅ CURVATURE PÅ BENDET
\begin{figure}[h!]
  \centering
