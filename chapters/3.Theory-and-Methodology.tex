...
The transient RDT-only simulation described here is a separate run from the \texttt{NoGV} reference of the design matrix (Section~\ref{sec:methods_rdt_baseline}). The two share the same RDT geometry and the same operating point but differ in three respects. The simulation here is transient and produces a time-averaged wake; the \texttt{NoGV} reference is steady. The simulation here is used as the design input for the MATLAB tool; the \texttt{NoGV} reference is used as the swirl-reduction baseline for the 18 simulations. The simulation here samples a single radial plane; the \texttt{NoGV} reference enters the full multi-plane post-processing pipeline. The two are kept distinct in this thesis and are not interchangeable.
