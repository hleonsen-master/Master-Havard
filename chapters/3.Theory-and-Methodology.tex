...
\begin{figure}[H]
  ...
  \caption[Velocity triangles at the RDT trailing edge]{Velocity triangles at the RDT trailing edge ($s>0.90$) for the six radial spans. Blue: absolute velocity $\vec{C}$; green: peripheral velocity $\vec{U}=\omega r$; red: relative velocity $\vec{W}=\vec{C}-\vec{U}$. The three vectors close as $\vec{C}=\vec{U}+\vec{W}$ by construction. Operating point: $N=650$~rpm, $Z_r=7$.}
  \label{fig:rdt_velocity_triangles_TE}
\end{figure}
 
\paragraph{Wake structure observed in the trailing-edge region.}
Three observations from Table~\ref{tab:rdt_te_values} are relevant for the wake characterisation. First, the meridional component $C_m$ remains close to $1.3$~m/s in the central span band ($r/R\in[0.20,0.80]$); it drops to $0.67$~m/s near the hub and to $0.99$~m/s near the tip. This is consistent with the low-momentum hub region and the tip-vortex interaction expected in a hubless rim-driven thruster~\cite{DjupeslandTrivedi2025}. Second, the tangential component $C_u$ increases nearly linearly with radius, from $3.57$~m/s at the hub to $13.85$~m/s at the tip, qualitatively consistent with the forced-vortex distribution targeted in the upstream design~\cite{DjupeslandTrivedi2025}. Third, the absolute flow angle $\alpha$ exceeds $80^\circ$ over most of the design span and reaches $85.9^\circ$ at $r/R=0.99$. The wake on the blade trailing edge is therefore dominated by tangential motion, in line with what the guide vanes are designed to turn back towards the axial direction.
