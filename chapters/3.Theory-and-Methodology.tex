...
A complementary identifier of coherent vortex cores is the second invariant of the velocity-gradient tensor. The tensor splits into a symmetric strain-rate part $\mathbf{S} = \tfrac{1}{2}(\nabla\vec{u} + \nabla\vec{u}^{T})$ and an antisymmetric vorticity part $\boldsymbol{\Omega} = \tfrac{1}{2}(\nabla\vec{u} - \nabla\vec{u}^{T})$. Hunt et al.~\cite{Hunt1988} define
\begin{equation}
Q \;=\; \tfrac{1}{2}\!\left(\|\boldsymbol{\Omega}\|^{2} - \|\mathbf{S}\|^{2}\right),
\label{eq:Q_def}
\end{equation}
so that $Q > 0$ identifies a region where local rotation dominates local strain. For incompressible flow, Eq.~\ref{eq:Q_def} is equivalent to
\begin{equation}
Q \;=\; \tfrac{1}{4}\,|\nabla\times\vec{u}|^{2} \;-\; \tfrac{1}{4}\,\mathrm{SSR}^{2},
\label{eq:Q_cfx}
\end{equation}
with $\mathrm{SSR} = \sqrt{2\,S_{ij}S_{ij}}$ as exported by CFX. Both terms carry the same prefactor. A pure shear flow gives $|\nabla\times\vec{u}|^{2} = \mathrm{SSR}^{2}$ and therefore $Q = 0$ identically, consistent with the underlying balance of rotation and strain.

Two consequences follow for the post-processing. Vortex extraction uses a positive threshold $Q_{\mathrm{ref}} > 0$, since the discrete cores of interest sit at $Q > 0$. In this thesis the threshold is set from the bulk-flow scale of the geometry as
\begin{equation}
Q_{\mathrm{ref}} \;=\; \left(\frac{U_{b}}{D}\right)^{2} \;\approx\; 21.5~\mathrm{s}^{-2},
\label{eq:Q_ref}
\end{equation}
with $U_{b} = 2.32~\mathrm{m\,s^{-1}}$ the bulk axial velocity and $D = 0.5~\mathrm{m}$ the pipe diameter. The same $U_{b}$ and $D$ apply to every case in the screening matrix, so the relative ranking between configurations does not depend on the absolute level chosen for the threshold. Wall-bounded regions show strongly negative $Q$ because the boundary layer is shear-dominated and $\|\mathbf{S}\|^{2}$ exceeds $\|\boldsymbol{\Omega}\|^{2}$ there. Negative $Q$ at walls is the expected signature of wall-bounded shear and is not a numerical artefact. The criterion complements $\lambda_{ci}$: both isolate vortex cores, but $Q$ measures the strain-rotation balance directly whereas $\lambda_{ci}$ uses the eigenvalue structure of $\nabla\vec{u}$.
...
A second factor moderates the comparison. The axial spacing between the RDT trailing edge and the GV leading edge is large compared with the compact axial-pump bound discussed in Section~\ref{sec:rsi-axial-spacing}. The present geometry has $a/L_{\mathrm{mid}} \approx 1.39$, about $9$ times the upper bound of the G\"ulich window (Section~\ref{sec:methods_axial_spacing}). The potential-interaction component of the rotor-stator pulsation is therefore expected to be weak by the time the flow reaches the GV row. The $m=0$ breathing-mode signature anticipated for $Z_r = Z_s = 7$ is correspondingly attenuated relative to the compact-stage textbook case. The transient diagnostic in Chapter~\ref{chap:results_discussion} tests whether the residual breathing-mode signature in \texttt{CC\_450\_7} is detectable above the spinning-mode background in \texttt{CC\_400\_6}. The compact-stage idealisation underlying the Tyler-Sofrin argument is only partly satisfied here, and the diagnostic is interpreted in that context.
...
The angle extracted on this plane should not be confused with the blade-surface velocity triangles reported in Section~\ref{sec:methods_rdt_velocity_triangles}. The velocity triangles in that section are sampled on the RDT blade surface near the trailing edge and include local blade-surface and boundary-layer effects. They show local angles above $80^\circ$ in the outer span. The guide vanes are placed downstream of the rotor, after the wake has mixed over approximately $0.20$~m. The design-input angle $\alpha_{\mathrm{base}}(r)$ is therefore taken from the downstream wake plane, not from the blade-surface trailing-edge samples.
...
\paragraph{Distinction from the \texttt{NoGV} reference.}
The transient RDT-only simulation is used only to construct the design-input angle $\alpha_{\mathrm{base}}(r)$. It is separate from the steady \texttt{NoGV} reference (Section~\ref{sec:methods_rdt_baseline}), which is used only as the swirl-reduction baseline for the post-processing metrics.
...
The axial spacing is most informatively normalised by the rotor chord $L$, following the convention of G\"ulich~\cite{Gulich2010} and Bijukchhe and Trivedi~\cite{Bijukchhe2026} discussed in Section~\ref{sec:rsi-axial-spacing}. The chord of the hubless RDT blade varies along the span from $L \to 0$ at the root ($r = 0.050$~m) to $L_{\max} = 197.0$~mm at the shroud ($r = 0.250$~m). The blade midspan lies at $r = 0.150$~m, where the chord is $L_{\mathrm{mid}} = 138.5$~mm. This midspan value is adopted as the representative chord for the comparison. The corresponding spacing ratio is
\begin{equation}
    \frac{a}{L_{\mathrm{mid}}} \;=\; \frac{0.192}{0.139} \;=\; 1.39,
    \label{eq:my_a_over_L}
\end{equation}
which exceeds the upper bound of the G\"ulich window $a/L = 0.05$-$0.15$ by a factor of about $9$. The conclusion is robust to the choice of representative chord. Using the largest chord at the shroud ($L_{\max} = 197.0$~mm) gives $a/L = 0.97$, still $6.5$ times the upper limit. Using the chord at the inner edge of the GV-overlap region ($r = 0.12$~m, $L \approx 116$~mm) gives $a/L = 1.66$, about $11$ times the upper limit. The bound~\eqref{eq:gulich_a_over_L} is therefore a qualitative reference for the importance of rotor-stator spacing in this geometry, not a numerical design constraint that should be satisfied. Two consequences for the analysis follow.
...
A common set of analysis planes is used for all 18 GV configurations and for the \texttt{NoGV} reference. The planes are listed in Table~\ref{tab:planes}. Axial planes from \texttt{GVStart} to \texttt{BendStart} are normal to the streamwise $z$ direction. The two post-bend planes, \texttt{BendEnd} and \texttt{ConeEnd}, are normal to the vertical $y$ direction.

\begin{table}[h!]
  \centering
  \caption[Analysis planes]{Common array of analysis planes used in the post-processing of all 18 configurations and the \texttt{NoGV} reference. The axial coordinate is defined relative to \texttt{GVStart}, with positive $z$ downstream. \texttt{BendEnd} and \texttt{ConeEnd} are defined in the vertical $y$ direction after the bend.}
  \label{tab:planes}
  \small
  \begin{tabular}{l c c c l}
    \toprule
    \textbf{Plane} & \textbf{$z$ [m]} & \textbf{$y$ [m]} & \textbf{Normal} & \textbf{Description} \\
    \midrule
    GVStart         & 0.000 & --    & $z$ & Start of the GV section; reference plane \\
    GV leading edge & 0.030 & --    & --  & Leading edge of the guide vanes \\
    GVEnd z680      & 0.495 & --    & $z$ & Plane 5 mm upstream of the GV-section outlet \\
    GVEnd2          & 0.710 & --    & $z$ & Downstream plane in the conical draft tube \\
    GVEnd3          & 0.910 & --    & $z$ & Downstream plane in the conical draft tube \\
    GVEnd4          & 1.110 & --    & $z$ & Downstream plane in the conical draft tube \\
    GVEnd5          & 1.310 & --    & $z$ & Downstream plane in the conical draft tube \\
    GVEnd6          & 1.510 & --    & $z$ & Downstream plane in the conical draft tube \\
    BendStart       & 2.000 & --    & $z$ & Start of the $90^\circ$ bend \\
    BendEnd         & --    & 0.497 & $y$ & End of the $90^\circ$ bend \\
    ConeEnd         & --    & 0.757 & $y$ & Final downstream plane before the RPT \\
    \bottomrule
  \end{tabular}
\end{table}
...
Even within the well-defined scope of this thesis, several limitations should be stated honestly:
\begin{itemize}
  \item \textbf{Single inlet wake.} All 18 configurations are designed against the same RDT wake. The present work therefore reflects the relative behaviour of the configurations under the specific inflow used, and the ranking may shift with a different RDT design or operating point.
  ...
  \item \textbf{Steady RANS.} The 18 simulations of the design matrix use steady RANS. Two separate transient simulations (\texttt{CC\_450\_7} and \texttt{CC\_400\_6}) are performed for the rotor-stator interaction analysis but are not part of the main ranking.
  \item \textbf{No multiphase cavitation modelling.} Cavitation is treated indirectly through the static-pressure field; vapour-fraction modelling is not performed in this work.
  ...
  \item \textbf{No structural assessment.} Strength, fatigue and vibration of the guide vanes are not analysed in this work.
...
\end{equation}
The values in Table~\ref{tab:rdt_te_values} therefore describe the local kinematics on the blade trailing-edge surface, sampled along the blade-loading line. They are not the free-stream wake field at the guide-vane leading-edge plane. The free-stream wake field at the GV leading-edge plane is the quantity used as the design input and is reported separately in Section~\ref{sec:methods_design_input_baseline}.
...
The meridional component $C_m$ starts at $-0.03$~m/s at the hub and rises to a peak of $1.67$~m/s in the central span band $r/R\in[0.60,\,0.80]$. It then drops to $0.21$~m/s at the tip. This C-shaped profile is consistent with the low-momentum hub region and the tip-vortex interaction expected in a hubless rim-driven thruster~\cite{DjupeslandTrivedi2025}. The tangential component $C_u$ increases monotonically with radius from $4.22$~m/s at the hub to $15.71$~m/s at the tip, qualitatively consistent with the forced-vortex distribution targeted in the upstream design~\cite{DjupeslandTrivedi2025}. The absolute flow angle $\alpha$ stays between $78^\circ$ and $90^\circ$ across the design span. It is closest to purely tangential at the endpoints ($90.3^\circ$ at the hub and $89.2^\circ$ at the tip) and reaches its lowest value of $77.8^\circ$ at $r/R = 0.40$. The wake on the blade trailing edge is therefore dominated by tangential motion, which is what the guide vanes are designed to redirect towards the axial direction.
 
\paragraph{Inner cut-out.}
The values for $r/R\in\{0.01,0.20\}$ correspond to radii $r=55.9$~mm and $r=93.2$~mm, both of which lie inside the inner cut-out of the guide-vane span $r\in[120,250]$~mm fixed by the radial backflow boundary (Section~\ref{sec:methods_design_input_baseline}). They are reported here as a wake description; they are not used to set the local guide-vane geometry.
