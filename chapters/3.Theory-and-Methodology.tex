...
The cost of this choice is that the screening reports the swirl-reduction performance achievable with a conservative slip correction, not the maximum achievable performance. Section~\ref{sec:results-fft} returns to this point when the swirl reduction is compared to the project-work Case B reference.
...
\begin{itemize}
  ...
  \item swirl number $S$ (Eq.~\ref{eq:S_def}) and its donut-restricted version $S_{\mathrm{donut}}$ over the design span $r \in [0.12, 0.25]$;
