...
\subsection{Bend secondary flow and Dean-vortex interaction}
\label{sec:bend-hl-collapse}

The bend analysis is included to explain why the bend loss changes strongly
between the NoGV and GV cases. It is not used as an independent ranking metric
for the guide-vane designs. The primary design ranking remains based on
\(\Delta H_{\mathrm{sys}}\), \(|\alpha|_{\mathrm{ConeEnd}}\), and
\(S_{\mathrm{ConeEnd}}\).

The \(90^\circ\) bend introduces secondary flow even without upstream swirl.
For the straight NoGV reference, this appears as a classical Dean-pair
structure with nearly zero net in-plane swirl. When the untreated RDT wake is
sent directly into the bend, the flow becomes swirl dominated instead. The
guide vanes therefore affect the bend primarily by reducing the tangential
momentum entering it.

\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{Figures/Results/dean_montage_BendEnd.png}
    \caption{Secondary-flow field at BendEnd for the straight reference, the
    NoGV RDT-swirl reference and four guide-vane configurations. Colours show
    the normalised in-plane velocity magnitude, \(|V_{\mathrm{ip}}|/U_b\),
    using one common absolute scale for all panels. White streamlines show the
    in-plane velocity field. The annotation in each panel gives the local peak
    value and the bend head loss from BendStart to BendEnd. The untreated RDT
    wake produces a much stronger bend response than either the straight
    reference or the guide-vane cases.}
    \label{fig:dean-bendend}
\end{figure}
Figure~\ref{fig:dean-bendend} separates the bend response into three regimes.
The straight NoGV case develops the expected Dean-pair structure. The RDT-swirl
NoGV case is dominated by the imposed tangential momentum. The long-chord
candidates \texttt{CC\_500\_8} and \texttt{CS\_500\_8} reduce the secondary
flow to a weak residual-swirl regime, while the short-chord cases
\texttt{CC\_250\_9} and \texttt{CS\_250\_9} retain a stronger coherent spiral.

\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{Figures/Results/dean_streamwise.png}
    \caption{Streamwise development of peak in-plane velocity and net
    secondary-flow swirl through the bend and two diameters downstream. Left:
    99.5th-percentile peak \(|V_{\mathrm{ip}}|/U_b\). Right:
    \(S_{\mathrm{net}}\) on the same cross-planes. The vertical dotted line
    marks the bend exit. The sampling stations are ordered planes and are not
    equally spaced in arclength, so the curves should be interpreted
    qualitatively.}
    \label{fig:dean-streamwise}
\end{figure}

Figure~\ref{fig:dean-streamwise} shows that the NoGV swirl reference remains
on a separate scale throughout the bend, with peak \(|V_{\mathrm{ip}}|/U_b\)
above 3 and \(S_{\mathrm{net}}\) above 1 at the bend inlet. The two long-chord
GV cases remain close to the straight-flow reference, whereas the two
short-chord contrast cases stay near \(|V_{\mathrm{ip}}|/U_b \approx 1\) and
\(S_{\mathrm{net}}\approx0.4\text{--}0.6\). This confirms that the bend-loss
collapse is mainly a NoGV-versus-GV effect. Removing the strong RDT swirl
reduces \(HL_{\mathrm{bend}}\) from approximately \(0.37\) m to below
\(0.03\) m for the shown GV cases.

The remaining differences between GV cases are small compared with this
collapse and should not be used as a primary ranking metric. The streamwise
curves also show that the interaction is not strictly additive. Local peaks can
occur inside the bend, even when the bend-exit field is weak. The additional
BendStart and downstream contour fields are provided in
Appendix~\ref{app:dean-panels}.
...
\hline
Case & $|\alpha|_{\mathrm{ConeEnd}}$ (deg) & $S_{\mathrm{GVStart}}$ & $S_{\mathrm{GVEnd6}}$ \\ 
\hline
\texttt{CS\_500\_8} & $9.1$  & $1.22$ & $0.15$ \\
\texttt{CS\_500\_6} & $13.1$ & $1.24$ & $0.20$ \\
\texttt{CS\_450\_8} & $16.7$ & $1.12$ & $0.37$ \\
\texttt{CS\_450\_6} & $22.3$ & $1.01$ & $0.55$ \\
\texttt{CS\_400\_8} & $22.2$ & $0.94$ & $0.52$ \\
\texttt{CS\_400\_6} & $29.3$ & $0.94$ & $0.63$ \\
\texttt{CS\_350\_8} & $28.1$ & $0.93$ & $0.65$ \\
\texttt{CS\_300\_9} & $30.7$ & $0.90$ & $0.77$ \\
\texttt{CS\_250\_9} & $35.1$ & $0.87$ & $0.94$ \\
\texttt{CC\_500\_8} & $16.5$ & $0.96$ & $0.31$ \\
\texttt{CC\_500\_6} & $17.3$ & $0.91$ & $0.38$ \\
\texttt{CC\_450\_8} & $20.6$ & $0.84$ & $0.47$ \\
\texttt{CC\_450\_6} & $25.5$ & $0.85$ & $0.59$ \\
\texttt{CC\_400\_8} & $21.5$ & $0.88$ & $0.51$ \\
\texttt{CC\_400\_6} & $28.2$ & $0.88$ & $0.74$ \\
\texttt{CC\_350\_8} & $25.1$ & $0.87$ & $0.60$ \\
\texttt{CC\_300\_9} & $21.4$ & $0.86$ & $0.69$ \\
\texttt{CC\_250\_9} & $35.0$ & $0.82$ & $0.88$ \\
NoGV swirl          & $63.72$ & $1.67$ & $2.38$ \\
\hline
\end{tabular}
\end{table}
...
Three of the four are present in the anchor sub-sample: \texttt{CS\_500\_8}, \texttt{CS\_500\_6}, and \texttt{CC\_500\_8}. \texttt{CC\_400\_8} was not rerun. The discussion that follows uses the corrected data for the three anchored cases and notes the gap for \texttt{CC\_400\_8} explicitly.
...
\section{Downstream flow conditioning: cone, bend, Dean vortices}
\label{sec:downstream-conditioning}

The screening matrix has so far been characterised at \texttt{ConeEnd}, the plane that defines the RPT inlet condition. The path from \texttt{GVEnd} through the contraction and the $90^\circ$ bend is not passive transport. The cone redistributes the residual swirl as the cross-section narrows, and the bend generates its own secondary flow that interacts with whatever angular momentum arrives at \texttt{BendStart}. This section resolves the forward references made earlier in the chapter: the bend head-loss collapse mechanism flagged in Section~\ref{sec:hl-decomposition}, the structure of the Dean-vortex residual flagged in Section~\ref{sec:nogv-straight-validation}, and the streamwise development of secondary flow that ties the two effects together.

\subsection{Bend head-loss collapse mechanism}
\label{sec:bend-hl-collapse}

The bend produces $0.393$~m of head loss in the NoGV swirl reference and $0.014$ to $0.058$~m in any GV configuration, an order-of-magnitude reduction. The cause is the way a $90^\circ$ bend responds to upstream swirl. On a near-axial flow the bend generates only its intrinsic Dean-vortex pair, a closed-loop secondary flow that dissipates locally with modest loss. The straight validation in Section~\ref{sec:nogv-straight-validation} samples this case, with peak in-plane velocity $|V_{\mathrm{in\text{-}plane}}|/U_b = 0.51$ and area-averaged swirling strength $2.40$~s$^{-1}$ at \texttt{ConeEnd}. On a strongly swirling flow the symmetric pair cannot form: an asymmetric tilted structure develops and the bend converts the residual rotational energy into mixing loss. The NoGV reference samples this regime, with swirling strength reaching $25.1$~s$^{-1}$.

Every GV configuration delivers a flow with $|S|_{\mathrm{ConeEnd}} \leq 1.15$ to the bend, well below the NoGV value of $5.35$, so the bend stays in its low-swirl regime and the loss converges to the Dean-only floor regardless of GV design. The collapse is therefore binary: it depends on whether a guide vane is present, not on which one. Graded effects of GV quality appear not in $\mathrm{HL_{bend}}$ but in how strongly the bend amplifies the residual swirl number, examined next.
...
\subsection{Dean-vortex structure at BendEnd}
\label{sec:dean-vortex}

The straight validation in Section~\ref{sec:nogv-straight-validation} isolates
the bend-induced secondary flow with no upstream swirl source. The result is a
classical Dean-pair structure with nearly zero net in-plane swirl. When the
untreated RDT wake enters the bend, the imposed tangential momentum dominates
the bend response and the flow becomes swirl dominated. The guide vanes affect
the bend mainly by reducing this incoming tangential momentum.

\begin{figure}[H]
\centering
\includegraphics[width=0.99\linewidth]{Figures/Results/dean_montage_BendEnd.png}
\caption{Secondary-flow field at BendEnd for the straight reference, the
NoGV RDT-swirl reference and four guide-vane configurations. Colours show the
normalised in-plane velocity magnitude, \(|V_{\mathrm{ip}}|/U_b\), using one
common absolute scale for all panels. White streamlines show the in-plane
velocity field. The annotation in each panel gives the local peak value and
the bend head loss from BendStart to BendEnd. The untreated RDT wake produces
a much stronger bend response than either the straight reference or the
guide-vane cases.}
...
\centering
\small
\caption{Secondary-flow indicators for the six bend cases.
\(S_{\mathrm{net,B00}}\) is evaluated at Bend00. The peak value is the maximum
99.5th-percentile \(|V_{\mathrm{ip}}|/U_b\) over the sampled stations from
Bend00 to Down2D. \(HL_{\mathrm{bend}}\) is the head loss from BendStart to
BendEnd.}
...
Figure~\ref{fig:dean-montage} and Table~\ref{tab:dean-secondary} separate the
cases into three regimes. The straight NoGV case has almost zero net swirl at
the bend inlet and develops the expected Dean-pair structure. Its peak
\(|V_{\mathrm{ip}}|/U_b\) remains below 0.5. The RDT-swirl NoGV case is instead
swirl dominated, with \(S_{\mathrm{net,B00}}=1.735\) and a peak in-plane
velocity of about 3.27 times the bulk velocity. This confirms that the
untreated RDT wake changes the bend from a weak secondary-flow element into a
strongly swirl-dominated part of the duct.

The long-chord candidates \texttt{CC\_500\_8} and \texttt{CS\_500\_8} both
reduce \(S_{\mathrm{net,B00}}\) to approximately 0.13. Their BendEnd patterns
are not identical, but both are much closer to the straight-flow reference than
to the NoGV swirl reference. The short-chord cases \texttt{CC\_250\_9} and
\texttt{CS\_250\_9} retain \(S_{\mathrm{net,B00}}\approx0.5\) and keep peak
in-plane velocities around one bulk velocity, showing that they under-turn the
RDT wake before the bend.

The bend-loss response is therefore mainly a NoGV-versus-GV effect. Removing
the strong RDT swirl reduces \(HL_{\mathrm{bend}}\) from 0.37 m to below
0.03 m for the shown GV cases. The remaining differences between GV cases are
small compared with this collapse and should not be used as a primary ranking
metric. The streamwise data in Appendix~\ref{app:dean-panels} show that the
interaction is not strictly additive: local peaks may occur inside the bend
even when the bend-exit field is weak.

\subsection{Vortex-core identification via Q-criterion (placeholder) La denne stå for det kommer noe her}
\label{sec:q-criterion-analysis}

% =====================================================================
% TODO -- Q-criterion analysis. Held empty pending Ch4 length budget.
%
% Planned scope (4 cases, 2 planes):
%   Cases : NoGV swirl, CS_500_8, CC_500_8, CC_250_9
%   Planes: ConeEnd, BendEnd
%
% CFX-Post expression (verified factor 0.25 on BOTH terms;
% see Section~\ref{sec:theory_swirl_metrics_def}, Q-criterion paragraph):
%   Q = 0.25*(Velocity.Curl X^2 + Velocity.Curl Y^2 + Velocity.Curl Z^2)
%     - 0.25*Shear Strain Rate^2
%
% Threshold for vortex extraction:
%   Q_ref ~ (U_bulk/D)^2 ~ 20 s^-2 in downstream region.
%
% Planned outputs:
%   1. One 2x4 contour figure (rows = ConeEnd / BendEnd, cols = 4 cases)
%   2. Small table : Q_max, A(Q>Q_ref)/A_total, N_cores per panel
%   3. 3-4 sentences linking to sec:dean-vortex and sec:s-streamwise
%
% Decision point: include only if total Ch4 length stays within budget.
% If cut, remove this subsection. The theory paragraph in
% sec:theory_swirl_metrics_def remains harmless as background.
% =====================================================================
...
This section verifies that the Pareto-front configurations achieve their bulk ranking through proportionate recovery across the radial span, rather than through outer-band recovery alone. Figure~\ref{fig:radial-profiles} shows three quantities at \texttt{ConeEnd} for the NoGV swirl reference and four guide-vane configurations: \texttt{CS\_500\_8}, \texttt{CC\_500\_8}, \texttt{CC\_400\_8}, and \texttt{CC\_250\_9}. The plotted quantities are the absolute flow angle $|\alpha|(r/R)$, the tangential velocity $C_u(r/R)$, and the meridional velocity $C_m(r/R)$.
...
\item Identification of the non-additive interaction between residual GV-induced swirl and bend-induced Dean structure as a candidate mechanism for the lower-than-Dean peak in-plane velocity observed in the Pareto-front configurations (Section~\ref{sec:dean-vortex}).
...
Section~\ref{sec:dean-vortex} identifies a non-additive interaction between residual swirl and the bend-induced Dean structure but does not decompose the secondary flow into rotational components. A targeted post-processing of the secondary-flow field on the corrected-mesh anchor cases would test whether the partial-cancellation mechanism specifically applies, and whether NPR-sign cases enter the same interaction regime as PPR-sign cases.
