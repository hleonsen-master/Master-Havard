...
The bend analysis is included to explain why the bend loss changes strongly between the NoGV and GV cases. It is not used as an independent ranking metric for the guide-vane designs. The primary design ranking remains based on \(\Delta H_{\mathrm{sys}}\), \(|\alpha|_{\mathrm{ConeEnd}}\), and \(S_{\mathrm{ConeEnd}}\).

The \(90^\circ\) bend introduces secondary flow even without upstream swirl. For the straight NoGV reference, this appears as a classical Dean-pair structure with nearly zero net in-plane swirl. When the untreated RDT wake is sent directly into the bend, the flow becomes swirl dominated instead. The guide vanes therefore affect the bend primarily by reducing the tangential momentum entering it.
...
Figure~\ref{fig:dean-streamwise} shows that the NoGV swirl reference remains on a separate scale throughout the bend, with peak \(|V_{\mathrm{ip}}|/U_b\) above 3 and \(S_{\mathrm{net}}\) above 1 at the bend inlet. The two long-chord GV cases remain close to the straight-flow reference, whereas the two short-chord contrast cases stay near \(|V_{\mathrm{ip}}|/U_b \approx 1\) and \(S_{\mathrm{net}}\approx0.4\text{--}0.6\). This confirms that the bend-loss collapse is mainly a NoGV-versus-GV effect. Removing the strong RDT swirl reduces \(HL_{\mathrm{bend}}\) from approximately \(0.37\) m to below \(0.03\) m for the shown GV cases.

The remaining differences between GV cases are small compared with this collapse and should not be used as a primary ranking metric. The streamwise curves also show that the interaction is not strictly additive. Local peaks can occur inside the bend, even when the bend-exit field is weak. The additional BendStart and downstream contour fields are provided in Appendix~\ref{app:dean-panels}.
...
The shortlist spans the useful corners of the design space. \texttt{CS\_500\_8} delivers the lowest residual angle in the matrix and the most favourable radial angle profile, but it also has the highest total head loss among the Pareto cases. \texttt{CC\_400\_8} gives the highest system head increase, but leaves a larger residual angle at \texttt{ConeEnd}. \texttt{CC\_500\_8} provides the best head-preserving compromise among the anchor-tested cases, with a moderate residual angle and substantially lower total head loss than the CS long-chord cases. \texttt{CS\_500\_6} remains on the uncorrected Pareto front, but its position is less robust after the corrected-anchor comparison discussed in Section~\ref{sec:pareto-anchor-check}.
