...
Two transient RDT--GV simulations were run at the nominal operating point ($n = 650$~rpm) to test the Tyler--Sofrin prediction directly: \texttt{CC\_450\_7} ($Z_r=Z_s=7$, $m_{\min}=0$) as the breathing-mode reference, and \texttt{CC\_400\_6} ($Z_s=6$, $m_{\min}=1$) as the spinning-mode comparator. The case selection is motivated in Section~\ref{sec:rsi-thesis}. Both runs were initialised from the same baseline restart at $t = 2.447$~s and continued for 15 rotor revolutions to $t = 3.831$~s.
...
\texttt{CC\_400\_6} ($m_{\min} = 1$) does not show systematically lower pulsation levels than \texttt{CC\_450\_7} ($m_{\min} = 0$). The amplitudes are of the same order of magnitude across all signals.

\subsection{Physical interpretation: large axial gap and non-uniform inflow}
\label{sec:results-fft-physicalexpl}

The absent breathing-mode signature is consistent with the large axial gap documented in Section~\ref{sec:methods_axial_spacing}, where $a/\bar{L} \approx 1.13$ places the geometry well outside the compact-stage window in which the Tyler--Sofrin argument is sharpest. The wake mixes and redistributes before reaching the vanes, weakening the coherent $m = 0$ build-up.
