...
The bend section produces $0.393$~m of head loss in the NoGV swirl reference and $0.014$ to $0.058$~m in any GV configuration. The order-of-magnitude reduction is the most pronounced single effect in the head-loss decomposition. The mechanism is rooted in how a $90^\circ$ bend responds to upstream swirl.

A bend acting on a near-axial flow generates only its intrinsic Dean-vortex pair from the imbalance between centrifugal acceleration and radial pressure gradient. The associated head loss is modest. The bend-induced secondary flow is a closed-loop pattern that dissipates locally without large-scale mixing. The straight validation in Section~\ref{sec:nogv-straight-validation} samples exactly this case: an axial inlet, no upstream swirl source, and a residual Dean structure at \texttt{ConeEnd} with peak in-plane velocity $|V_{\mathrm{in\text{-}plane}}|/U_b = 0.51$ and area-averaged swirling strength $2.40$~s$^{-1}$.

A bend acting on a strongly swirling flow produces a qualitatively different loss. The residual rotation arriving at \texttt{BendStart} interacts with the centrifugal field of the bend. The symmetric Dean pair cannot establish itself. Instead, an asymmetric tilted vortex structure develops, and the bend converts the residual rotational kinetic energy into mixing loss through extensive cross-section transport rather than carrying it as coherent angular momentum. The NoGV swirl reference samples this regime: swirling strength at \texttt{ConeEnd} reaches $25.1$~s$^{-1}$, an order of magnitude larger than the Dean-only case.

The screening matrix sits between these two extremes. The GV passage removes enough angular momentum that the bend receives a flow with $|S|_{\mathrm{ConeEnd}} \leq 1.15$ for every configuration, well below the NoGV corrected value of $5.35$. The bend therefore operates closer to its low-swirl regime in every GV case, and the head loss converges to the Dean-only floor regardless of which GV design generated the residual. The bend collapse is essentially binary: GV present versus absent. Graded effects of GV quality appear not in $\mathrm{HL_{bend}}$ but in the swirl-number amplification factor through the bend, examined next.

\subsection{Streamwise evolution of the swirl number}
\label{sec:s-streamwise}

Figure~\ref{fig:s-streamwise} shows the streamwise evolution of $|S|$ from \texttt{GVStart} to \texttt{BendEnd} for four representative configurations: the NoGV swirl reference, \texttt{CS\_500\_8} (best Pareto on residual angle), \texttt{CC\_500\_8} (anchor-confirmed Pareto on head increase), and \texttt{CC\_250\_9} (worst residual angle in the matrix).

\begin{figure}[H]
\centering
\includegraphics[width=0.95\linewidth]{Figures/Results/fig_4_6_S_streamwise.png}
...
The four traces sit at different vertical levels and follow different shapes. NoGV starts at $|S| \approx 1.8$ in the donut region, drops through what would have been the GV passage, then climbs through the cone and reaches $|S| = 6.6$ at \texttt{BendEnd} on the full-pipe scale. The bend dramatically amplifies a flow that arrives already strongly swirling.

\texttt{CS\_500\_8} drops sharply from $|S|_{\mathrm{GVStart}} = 1.22$ to $|S|_{\mathrm{GVEnd6}} = 0.15$, stays near $|S| = 0.17$ at \texttt{ConeEnd}, and ends at $|S| = 0.11$ at \texttt{BendEnd}. The bend slightly attenuates rather than amplifies. The residual swirl is weak enough that the bend's intrinsic Dean structure dominates and partially cancels the imposed rotation.

\texttt{CC\_500\_8} drops less sharply at \texttt{GVEnd6} ($|S| = 0.31$) but recovers similarly through the cone ($|S|_{\mathrm{ConeEnd}} = 0.13$) and ends at $|S|_{\mathrm{BendEnd}} = 0.23$. The bend nearly doubles the magnitude. The amplification pattern matches the head-corner-case position on the Pareto front: the bend penalises this configuration mildly compared to the angle-corner case.

\texttt{CC\_250\_9} barely drops at \texttt{GVEnd6} ($|S| = 0.88$). The short chord and high vane count produce a vane stack that under-turns and leaves the donut-region swirl close to its incoming value. Through the cone, $|S|$ grows to $1.05$ (NPR direction). Through the bend, $|S|$ nearly doubles again to $1.96$. The worst-recovery case in the matrix carries roughly one order of magnitude more angular momentum through the bend than the Pareto candidates.

The bend amplifies what it receives, and the amount of amplification scales with the magnitude of the incoming swirl. The screening metric at \texttt{ConeEnd} therefore understates the swirl carried into the downstream RPT system for the weak-recovery cases. Residual swirl is not just a local penalty at the RPT inlet; it is a quantity that grows downstream of the screening sampling plane.

\subsection{Dean vortex structure: straight validation and screening cases}
\label{sec:dean-vortex}

The straight validation in Section~\ref{sec:nogv-straight-validation} produces a near-axial flow through the cone and bend, with the only secondary structure being the bend-induced Dean pair. Figure~\ref{fig:dean-vortex} shows the in-plane velocity vector field and the swirling-strength contour at \texttt{ConeEnd} for this validation case.

\begin{figure}[H]
\centering
\includegraphics[width=0.95\linewidth]{Figures/Results/fig_4_7_dean_vortex.png}
\caption{Dean-vortex structure at \texttt{ConeEnd} for the NoGV straight validation. Left: in-plane velocity vector field showing the symmetric counter-rotating pair. Right: swirling-strength contour with two compact regions of opposite sign separated by the bend symmetry plane. Peak in-plane velocity reaches $|V_{\mathrm{in\text{-}plane}}|/U_b = 0.51$. No external swirl source is present; the structure is generated entirely by the $90^\circ$ bend acting on an axial inlet flow.}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.99\linewidth]{Figures/Results/fig_4_8_streamwise_composite.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\linewidth]{Figures/Results/fig_4_9_radial_profiles.png}
\caption{Radial profiles at \texttt{ConeEnd} for the NoGV swirl reference and four guide-vane configurations. Top: absolute flow angle $|\alpha|(r/R)$. Middle: tangential velocity $C_u(r/R)$. Bottom: meridional velocity $C_m(r/R)$. The radial coordinate is normalised by the local pipe radius $R$ at \texttt{ConeEnd}. Bulk $|\alpha|_{\mathrm{ConeEnd}}$ values for context: NoGV $63.7^\circ$ (corrected), \texttt{CS\_500\_8} $9.1^\circ$, \texttt{CC\_500\_8} $16.5^\circ$, \texttt{CC\_400\_8} $21.5^\circ$, \texttt{CC\_250\_9} $35.0^\circ$.}
...
Table~\ref{tab:final-decision} summarises the screening result for the four candidate configurations that emerged through the chapter. Three are anchor-tested or anchor-confirmed; one is an uncorrected-only candidate retained as the principal follow-up case.

\begin{table}[H]
\centering
\small
\caption{Final candidate shortlist with multi-criteria evaluation. Values are reported from the uncorrected $18$-case screening matrix for consistency, since \texttt{CC\_400\_8} lacks a corrected-mesh anchor. $S_{\mathrm{ConeEnd}}$ sign follows the convention of Section~\ref{sec:sign-of-swirl}: positive denotes residual rotation in the same sense as the upstream RDT swirl (PPR), negative denotes over-correction (NPR). Anchor status refers to the corrected-mesh validation layer of Section~\ref{sec:rdt-mesh-overlap-impact}.}
\label{tab:final-decision}
\begin{tabular}{l l rrr l l}
\hline
Case & Role & $\Delta H_{\mathrm{sys}}$ (m) & $|\alpha|$ (deg) & $S_{\mathrm{ConeEnd}}$ & Anchor & Recommendation \\ \hline
\texttt{CS\_500\_8} & Best flow alignment      & $3.23$ & $9.1$  & $+0.17$ (PPR) & confirmed    & Primary candidate         \\
\texttt{CC\_500\_8} & Best head, anchor-supported & $3.74$ & $16.5$ & $+0.13$ (PPR) & confirmed    & Primary candidate         \\
\texttt{CC\_400\_8} & High-head NPR case        & $3.75$ & $21.5$ & $-0.26$ (NPR) & not tested   & Highest-priority follow-up \\
\texttt{CS\_500\_6} & Uncorrected intermediate  & $3.33$ & $13.1$ & $+0.29$ (PPR) & inconclusive & At-risk, not rejected     \\
\hline
\end{tabular}
\end{table}

The shortlist spans the design space at its useful corners. \texttt{CS\_500\_8} delivers the lowest residual angle in the matrix and the most uniform radial profile, at the cost of the highest GV-passage head loss. \texttt{CC\_500\_8} delivers the highest anchor-confirmed head increase and accepts a moderate outer-span angle. \texttt{CC\_400\_8} sits at the head corner of the uncorrected Pareto front and combines high $\Delta H_{\mathrm{sys}}$ with NPR-direction residual swirl, but lacks corrected-mesh confirmation. \texttt{CS\_500\_6} retains its Pareto position in the uncorrected matrix but is at risk of domination by \texttt{CC\_500\_8} under correction, as discussed in Section~\ref{sec:pareto-anchor-check}.
