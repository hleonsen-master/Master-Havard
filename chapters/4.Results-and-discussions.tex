...
Two transient RDT--GV simulations were run at the nominal operating point ($n = 650$~rpm) to test the Tyler--Sofrin prediction directly. The cases were selected as diagnostic blade-count configurations, not as final hydraulic candidates. Configuration \texttt{CC\_450\_7} ($Z_r=Z_s=7$, $m_{\min}=0$) is used as the breathing-mode reference, while \texttt{CC\_400\_6} ($Z_s=6$, $m_{\min}=1$) is used as the spinning-mode comparator. The case selection is motivated in Section~\ref{sec:rsi-thesis}. Both runs were initialised from the same baseline restart at $t = 2.447$~s and continued for 15 rotor revolutions to $t = 3.831$~s.
...
Peak amplitudes at $f_n$ and $f_{\mathrm{BPF}}$ for all monitor signals are listed in Tables~\ref{tab:peak_amplitudes_450_7} and~\ref{tab:peak_amplitudes_400_6}; the ratio $A(f_{\mathrm{BPF}})/A(f_n)$ indicates the dominant forcing mechanism.
...
\texttt{CC\_400\_6} ($m_{\min} = 1$) does not show systematically lower pulsation levels than \texttt{CC\_450\_7} ($m_{\min} = 0$). The amplitudes are of the same order of magnitude across all signals. A likely explanation is discussed in Section~\ref{sec:results-fft-physicalexpl}.

\subsection{Physical interpretation: large axial gap and non-uniform inflow}
\label{sec:results-fft-physicalexpl}

The absent breathing-mode signature is consistent with the large axial spacing documented in Section~\ref{sec:methods_axial_spacing}. The relevant compact-stage parameter is the rotor--stator spacing normalised by the RDT rotor chord, $a/L$, not by the duct diameter. Using the span-averaged RDT chord in the GV-overlap region, $\bar{L}=169.8$~mm, gives $a/\bar{L}=0.192/0.170=1.13$. This is about 7.5 times larger than the upper limit of the compact axial-pump guideline $a/L=0.05$--$0.15$. The potential-interaction component of the rotor--stator pulsation is therefore expected to be strongly attenuated before the flow reaches the guide-vane row. The wake can therefore mix and redistribute before reaching the vanes, weakening the coherent $m = 0$ build-up predicted for a compact rotor--stator stage.
...
This interpretation is also consistent with the dominant sub-rotor-frequency content in the monitor signals. Low-frequency fluctuations are the signature of slowly evolving recirculation, not deterministic blade-passing interaction.
