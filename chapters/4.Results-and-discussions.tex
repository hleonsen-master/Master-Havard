...
The straight validation in Section~\ref{sec:nogv-straight-validation} isolates the bend-induced secondary flow with no upstream swirl source. Figure~\ref{fig:dean-vortex} shows the in-plane vectors and the swirling-strength contour at \texttt{ConeEnd}.
...
compared directly.
=======
The pattern is a symmetric counter-rotating Dean pair with peak in-plane velocity $|V_{\mathrm{in\text{-}plane}}|/U_b = 0.51$, and two compact regions of opposite swirling strength split by the bend symmetry plane. The $90^\circ$ bend acting on an axial inlet produces the whole structure with no external rotational source.

Figure~\ref{fig:streamwise-composite} compares \texttt{ConeEnd} for the NoGV swirl reference and three GV configurations: \texttt{CS\_500\_8}, \texttt{CC\_500\_8}, and \texttt{CC\_250\_9}.
>>>>>>> d125d34 (Update chapter 4 results and Dean vortex diagnostics)
...
=======
The NoGV pattern at \texttt{ConeEnd} is a single coherent spiral, not a counter-rotating pair: the upstream RDT swirl is strong enough that the bend-induced Dean structure cannot form. The GV cases sit in between. \texttt{CS\_500\_8} reaches a peak of $0.35$, below both the NoGV case and the pure-Dean value of $0.51$. Linear superposition of the residual swirl and the bend-induced flow would give a peak above the straight-validation value on the same plane, not below it, so the two secondary-flow components interact non-additively. Partial cancellation between the imposed swirl and one Dean cell is a plausible mechanism, but confirming it would need a decomposition of the secondary-flow field that is outside the scope of this screening. \texttt{CC\_500\_8} shows the same pattern at slightly higher peak; \texttt{CC\_250\_9} keeps a coherent residual spiral resembling the NoGV regime at lower magnitude.

The peak in-plane velocity tracks the residual-angle ranking of Section~\ref{sec:swirl-recovery}: configurations that lower $|\alpha|_{\mathrm{ConeEnd}}$ also lower the \texttt{ConeEnd} peak through this non-additive interaction, while under-correcting configurations carry a coherent spiral through the whole duct. Whether NPR-sign cases such as \texttt{CC\_400\_8} enter the same interaction regime cannot be settled without a corrected-mesh streamwise analysis, noted as future work in Section~\ref{sec:pareto-anchor-check}.
>>>>>>> d125d34 (Update chapter 4 results and Dean vortex diagnostics)
