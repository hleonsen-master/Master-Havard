...
This chapter consolidates the theoretical framework and the methodological choices used in the present CFD study work. The first part establishes the governing equations, the cylindrical-coordinate description of the flow, the velocity triangles and the swirl metrics. It then develops the cascade theory used to design the guide vanes, the turbulence-model assumptions and the rotor-stator interaction theory underlied by transient analyses. The second part applies this framework to the present geometry. It covers the parametric MATLAB tool from the project work~\cite{Leonsen2025}, the 18-case design matrix, and the no-GV reference simulation. It then describes the multi-domain CFX setup, the mesh strategy, the Grid Convergence Index study and the spectral-analysis methodology used for the two transient simulations.
}}
\end{center}
...
\label{eq:incidence}
\end{equation}
A design requirement is to keep $i$ small in order to avoid leading-edge separation and the associated increase in losses.

\paragraph{Choice of flow angle for stationary guide-vane design.}
The guide vanes are stationary, so $\vec{W}\equiv \vec{C}$ at the GV leading edge. The absolute angle $\alpha(r)$ from~\eqref{eq:alpha_definition} is therefore the relevant angle and is used to set the GV metal angles. The relative angle $\beta(r)=\arctan(W_u/C_m)$ is reported in Section~\ref{sec:methods_rdt_velocity_triangles} only as a kinematic descriptor of the upstream rotor wake; it is not used in the GV design. The upstream RDT design~\cite{DjupeslandTrivedi2025} is in the rotor-relative frame and uses $\beta$.
...
For the NTNU RPT, Kerlefsen reports a required NPSH of approximately $1.3$-$1.4$~m for a net head $H_{\mathrm{net}} \approx 12$~m, giving $\mathrm{NPSH}_\mathrm{R}/H_{\mathrm{net}} \approx 0.11$-$0.12$, with a recommended margin of $20$-$25\,\%$ between available and required NPSH~\cite{Kerlefsen2025}. The Store2Hydro booster is intended to add approximately $H \approx 5$~m of head upstream of the RPT, which is comfortable as long as guide-vane losses are kept moderate. Any total-pressure loss in the simulations therefore reduces the cavitation margin and is charged against the design.
 
In CFD, cavitation is commonly modelled as a homogeneous mixture of liquid water and water vapour, with mass-transfer rates determined by a Rayleigh-Plesset-based source term. In the present work no cavitation modelling is performed in the simulations; the static pressure field is examined for the absence of regions with $p < p_v$, but multiphase modelling is left as future work for the selected configuration.
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.75\textwidth]{Figures/Methods/BaselinePlot1.png}
    \caption[]{}
    \label{fig:UxUt}
\end{figure}
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.75\textwidth]{Figures/Methods/BaselinePlot2.png}
    \caption[]{}
    \label{fig:UxUt}
\end{figure}
...
The cost of this choice is that the simulation reports the swirl-reduction performance achievable with a conservative slip correction, not the maximum achievable performance. This choice is revisited in Chapter~\ref{chap:results_discussion} when the swirl-reduction results are interpreted.
...
%Det er viktig for deg å skjønne at alle disse velocity trianglene her er triangler som er tatt rett fra bladets trailing edge med pressure distribution osv etc. MEN det viktige her er at det er ikke disse vinklene som ble brukt som en Guide vane design input vinkel. Det har jeg 
\begin{table}[H]
  \centering
