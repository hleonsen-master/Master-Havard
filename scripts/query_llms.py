...
def get_env_with_node() -> dict:
    """Return environment with Node.js added to PATH."""
    env = os.environ.copy()
...
def is_cli_available(cli_name: str) -> bool:
    ...
    if cli_name == "codex":
        return os.path.exists(CODEX_PATH)
...
def query_codex_cli(prompt: str, timeout: int = 90) -> Tuple[bool, str]:
    """
    Query Codex CLI non-interactively using 'codex exec <prompt>'.
    Returns (success, response_or_error).
    """
    try:
        result = subprocess.run(
